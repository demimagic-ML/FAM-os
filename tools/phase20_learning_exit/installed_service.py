"""Lifecycle wrapper for the installed verified-learning service."""
...
class InstalledLearningService:
    def __init__(
        self, installation, repository: Path, product_root: Path,
        run_root: Path, responses: tuple[str, ...],
    ) -> None:
        ...
        self._process = subprocess.Popen(
            (
                sys.executable,
                str(repository / "tools/phase20_learning_exit/service_process.py"),
    ...
    def wait_ready(self, timeout: float = 30) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            if self._process.poll() is not None:
                raise RuntimeError(self._failure("installed learning service exited"))
            time.sleep(0.05)
        raise TimeoutError(self._failure("installed learning service was not ready"))
