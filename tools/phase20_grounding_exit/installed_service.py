"""Lifecycle wrapper for installed grounded-answer qualification."""
...
class InstalledGroundingService:
    def __init__(
        ...
        self._process = subprocess.Popen(
            (
                sys.executable,
                str(repository / "tools/phase20_grounding_exit/service_process.py"),
    ...
    def wait_ready(self, timeout: float = 30) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            if self._process.poll() is not None:
                raise RuntimeError(self._failure("installed grounding service exited"))
            time.sleep(0.05)
        raise TimeoutError(self._failure("installed grounding service did not become ready"))
