"""Serve one authenticated, deliberately truncated mutual-TLS response."""
...
PARTIAL_SENTINEL = b"PARTIAL_REMOTE_OUTPUT_MUST_NEVER_PERSIST"
...
def main() -> int:
    ...
    try:
        ...
        if opened.recovery_required or storage.core is None:
            raise RuntimeError("installed partial server storage is unavailable")
        approvals = tuple(
            record.approval
    ...
    try:
        ...
        with connection:
            ...
            with context.wrap_socket(connection, server_side=True) as secured:
                ...
                declared_size = len(PARTIAL_SENTINEL) + 32
                secured.sendall(struct.pack("!I", declared_size) + PARTIAL_SENTINEL)
        document = {
            "authenticated_peer": asdict(peer),
            "request_content_bytes": len(request),
            "request_sha256": hashlib.sha256(request).hexdigest(),
            "declared_response_bytes": declared_size,
            "sent_response_bytes": len(PARTIAL_SENTINEL),
            "partial_sentinel": PARTIAL_SENTINEL.decode("ascii"),
            "complete_frame_sent": False,
        }
