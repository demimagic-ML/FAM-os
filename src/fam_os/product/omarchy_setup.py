...
class OmarchySetup:
    def __init__(
        ...
        *,
        ...
        self.plugin_url = plugin_url or os.environ.get(
            "FAM_OS_OMARCHY_PLUGIN_URL", DEFAULT_PLUGIN_URL,
        )
        self._run = run
        self._which = which
    ...
    def setup(
        self, *, enable_widget: bool = True, start: bool = True,
        ...
        if not capabilities.host.supported and not (
            allow_experimental
            and capabilities.host.support_level == "experimental"
        ):
            raise RuntimeError(
        ...
        self._write_manifest({
            ...
            "services": ["fam-os.service", "fam-os-desktop.service", "fam-os-usage.timer"],
        ...
        diagnosis = self._wait_for_diagnosis() if start else diagnose_omarchy(self.detector)
        onboarding = self._open_onboarding() if start and diagnosis.healthy else False
        configured = (
            service_enabled and desktop_installed and launcher and collector
            and (not enable_widget or widget_installed and widget_enabled)
            and (not start or diagnosis.healthy)
        )
        return OmarchySetupReceipt(
            "setup", configured,
            service_enabled, desktop_installed, widget_installed, widget_enabled,
            launcher, collector, provider,
            str(self.manifest_path), diagnosis.document(), tuple(issues),
            diagnosis.healthy, onboarding,
        )

    def repair(self, *, widget: bool = False, service: bool = False) -> OmarchySetupReceipt:
        ...
        launcher, collector = self._install_agent_commands()
        provider = self._write_service_environment(capabilities)
        service_enabled = self._enable_services(start=True) if service else self._service_enabled()
        ...
        return OmarchySetupReceipt(
            ...
            service_enabled, desktop_installed, widget_installed, widget_enabled,
            launcher, collector, provider,
            str(self.manifest_path), diagnosis.document(),
        )
    ...
    def _write_service_environment(self, capabilities) -> str:
        endpoint = next(
            (item for item in capabilities.inference if item.reachable and item.kind == "ollama"),
            None,
        )
        ...
        provider = (
            "codex-subscription"
            if selected is not None and selected.agent_id == "codex"
            and codex is not None and codex.available
            else "ollama"
        )
        values = {
    ...
    def _enable_services(self, *, start: bool) -> bool:
        ...
        return core.returncode == 0 and desktop.returncode == 0 and usage.returncode == 0

    def _service_enabled(self) -> bool:
        return self._systemctl("is-enabled", "fam-os.service", check=False).returncode == 0
    ...
    def _install_widget(self, *, update: bool = False) -> tuple[bool, bool]:
        ...
        if not existing:
            result = self._command((
                "omarchy", "plugin", "add", self.plugin_url,
                "--enable", "--yes",
            ), check=False)
        ...
        if target.is_dir():
            ...
            if result.returncode == 0:
                try:
                    ...
                    entries = records if isinstance(records, list) else records.get("plugins", [])
                    match = next((
    ...
    def _git_plugin_owned(self, target: Path) -> bool:
        ...
        return result.returncode == 0 and _same_git_url(
            result.stdout.strip(), self.plugin_url,
        )
    ...
    def _require_verified_plugin(self, target: Path) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix="plugin-trust-", dir=temporary_root,
        ) as directory:
            ...
            verified = self._command((
                "git", "-C", str(target), "verify-commit", "HEAD",
            ), check=False, env=environment)
            if verified.returncode != 0:
                raise RuntimeError("FAM plugin HEAD is not signed by the pinned release key")
    ...
    def _open_onboarding(self) -> bool:
        ...
        if executable is None:
            return False
        result = self._command((
            executable, "console", "--runtime-root",
            str(self.paths.fam_runtime_root), "--port", "8765",
        ), check=False)
    ...
    def _command(
        self, command: tuple[str, ...], *, check: bool = True,
        env: dict[str, str] | None = None,
    ):
        return self._run(
            command, capture_output=True, text=True, timeout=120, check=check,
            env=env,
        )
...
def _read_service_environment(path: Path) -> dict[str, str]:
    try:
        ...
        return {}
    allowed = {
        "FAM_OS_CODEX_MODEL", "FAM_OS_CONSOLE_PORT",
        "FAM_OS_ENGINEERING_MODEL", "FAM_OS_ENGINEERING_PROVIDER",
        "FAM_OS_EXTERNAL_OLLAMA", "FAM_OS_MODEL", "FAM_OS_OLLAMA_EXECUTABLE",
        "FAM_OS_OLLAMA_URL",
    }
    result = {}
